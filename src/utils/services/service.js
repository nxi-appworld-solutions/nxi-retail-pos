...
export const getProductDetail = async (productCode) => {
  const res = await fetch(
    `${API_URL}/GetProductMasterDetails/6?code=${productCode}`,
  );
  const result = await res.json();
...
export const getVchNo = async (vchType) => {
  ...
  return await api(`/GetVchNo?${query}`, "GET");
};
...
export const invoiceReport = async (
  vchType,
  startDate,
  endDate,
  customer,
  ...
  return await api(
    `/GetTransactionsReport/transactions/report?${query}`,
    "GET",
  );
};

export const invoiceReportSummary = async (
  vchType,
  startDate,
  endDate,
  customer,
...
export const dashboardSalePurchaseDayChart = async (range = "1Y") => {
  ...
  return await api(
    `/GetSalesPurchaseDayChart/dashboard/day-chart?${query}`,
    "GET",
  );
};
...
export const posCatalog = async () => {
  return await api("/Catalog/catalog", "GET");
};
